...
mod day2 {
    ...
    pub fn solve_basic(input: &[&str]) {
        let valid_count = input.iter().map(|s| PasswordEntry::from(s)).filter(PasswordEntry::valid_standard).count();

        println!("{}", valid_count)
    }

    pub fn solve_twist(input: &[&str]) {
        let valid_count = input.iter().map(|s| PasswordEntry::from(s)).filter(PasswordEntry::valid_twist).count();
...
fn main() {
    day4::solve_default(INPUT);
    day4::solve_twist(INPUT);
}

static INPUT: &'static str = "iyr:2015
